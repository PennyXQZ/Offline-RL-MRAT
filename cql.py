...
import sys
sys.path.append('/home/ubuntu/DT/gym-mrat/gym_mrat')
...
class TrainConfig:
    ...
    # device: str = "cuda"
    device: str = "cpu"
    env: str = "MRAT-v0"  # OpenAI gym environment name
    ...
    dataset_path = 'multipath_data.csv'
...
def get_dataset(dataset_path):
    ...
    dataset = {'observations': [], 'actions': [], 'rewards': [], 'terminals': []}

    # Extract and append values to the dataset
    for i in range(len(df)):
        observations = df.loc[i, ["SNR_5G", "SNR_WiFi", "SNR_LiFi", "Delay_5G", "Delay_WiFi", "Delay_LiFi"]].values
        actions = df.loc[i, ["DataRate_5G", "DataRate_WiFi", "DataRate_LiFi"]].values
        rewards = np.sum(df.loc[i, ["Throughput_5G", "Throughput_WiFi", "Throughput_LiFi"]].values)
        terminals = False  # Always set to False

        # Append values to the dataset
        dataset['observations'].append(observations)
    ...
    # Now, 'dataset' dictionary contains the required information
    return dataset

def get_qlearning_dataset(env, dataset=None, terminate_on_end=False, **kwargs):
    """
...
def train(config: TrainConfig):
    ...
    dataset = get_qlearning_dataset(env, dataset=get_dataset(config.dataset_path), seed=config.seed)
